    ...
    <div class="collapse navbar-collapse pt-2 pt-lg-0" id="navbarNav" ref="navbarCollapse">
      <ul class="navbar-nav">
        <li class="nav-item">
          <RouterLink
            to="/"
            class="nav-link fw-bold px-4 px-lg-3 py-3"
            :class="{ active: status === 'home' }"
            >首頁</RouterLink
          >
        </li>
        <li class="nav-item">
      ...
      <ul class="d-none d-lg-flex navbar-nav ms-auto me-2">
        <li class="nav-item position-relative">
          <RouterLink to="/cart" class="nav-link px-3" :class="{ active: status === 'cart' }">
            ...
            <div
              class="bg-danger text-white rounded-circle text-center position-absolute num"
              v-if="cartNum !== 0"
            >
              {{ cartNum }}
            </div>
